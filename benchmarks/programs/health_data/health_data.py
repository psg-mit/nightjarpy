...
class Condition(BaseModel):
    name: str
    startDate: datetime
    status: Literal["active", "recurrence", "relapse", "inactive", "remission", "resolved", "unknown"]
...
@nj.fn
def main(user_messages: List[str]):
...
- At least respond with an OK message.""",
    ...
    for user_message in user_messages:
        ...
        # Assistant
        """natural
        Look at <history> to see the history of the conversation. If there is enough information to create a fully valid <HealthData> object (no missing required fields), save the health data to <:health_data> a <HealthData> object and save a response to the user in <:assistant_message> that acknowledges their given information.
...
def run() -> Tuple[Dict[str, Tuple[Any, Any]], Dict[str, Any], Dict[str, bool]]:
    ...
    correct_health_data = HealthData(
        medication=[],
        condition=[
            Condition(name="foot", startDate=datetime(2001, 1, 1), status="resolved", endDate=datetime(2002, 1, 1))
        ],
        other=[],
    )
    hard_results = {
